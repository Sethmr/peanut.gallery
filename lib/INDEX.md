...
| File | Purpose |
|---|---|
| [`director.ts`](director.ts) | Rule-based cascade director. Scores the transcript, picks a lead persona, cascades to others with decreasing probability and staggered delays. Owns `director_decision` log line. v1.5: `decide(…, opts?: { llmPick })` substitutes the primary when the Smart Director v2 race wins — cascade/cooldown/recency bookkeeping is unchanged. `TriggerDecision.source` is `"rule" \| "llm"`. |
| [`director-llm.ts`](director-llm.ts) | v1.5 — Smart Director v2 routing module. `pickPersonaLLM(ctx)` runs a short Claude Haiku completion (JSON-only response), validates the returned archetype, and resolves null on timeout / upstream error / malformed output. Never throws. Caller in `/api/transcribe` races this against `AbortSignal.timeout(400)`. Gated by `ENABLE_SMART_DIRECTOR=true`. |
