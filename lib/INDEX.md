...
| [`director-llm-v3-cerebras-v3prompt.ts`](director-llm-v3-cerebras-v3prompt.ts) | Shadow call against Cerebras Llama 3.1 8B (`llama3.1-8b`) via raw fetch to their OpenAI-compatible endpoint. Uses the v3 prompt (5-slot + confidence) with `response_format: { type: "json_schema" }` for hard-enum enforcement on `personaId`. Fire-and-forget; never feeds `director.decide`. Gated by `ENABLE_SMART_DIRECTOR_V3_CEREBRAS_V3PROMPT=true`. The sibling Groq shadow module was retired on 2026-06-03 along with the `groq-sdk` dependency (SET-11 was never reactivated). |
