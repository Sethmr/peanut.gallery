...
2. [`docs/SESSION-NOTES-2026-04-21-opus-catchup.md`](SESSION-NOTES-2026-04-21-opus-catchup.md) — **most recent handoff.** Opus catch-up after the Sonnet daemon run: merged #67 (SET-10 prompt-cache padding) + #68 (v3 Director + Cerebras shadow); held #61 (SET-8 in-band re-roll) with UX-regression review; filed SET-12 through SET-18 for the Director-finishing path. Points at `STATE-OF-DIRECTOR` as the canonical Director state.
...
7. [`docs/ROADMAP.md`](ROADMAP.md) — version-staged plan with fleshed-out sub-steps for near-term releases. Shipped: v1.2 / v1.3 / v1.4 / v1.5.0 (tagged 2026-04-19, in CWS review) / v1.5.1 (merged to `main` 2026-04-20 via PR [#18](https://github.com/Sethmr/peanut.gallery/pull/18); tag pending). In flight: v1.5.2 "First Run" — first-run Editor's Note tutorial walking users through the v1.5.1 settings drawer. Still outstanding from v1.5.0: the Smart Director server-flag canary (`ENABLE_SMART_DIRECTOR=true` for 48h) — slipped out of the v1.5.0 ship window; will decide a later point-release vs. v1.7 GA based on results. Path to v2.0 (Seth's explicit sequence, 2026-04-19): v1.6 Settings Pane (now a polish-and-a11y pass — core settings surface landed in v1.5.1) → v1.7 Smart Director GA → v1.8 Peanut Mascots → v1.9 Bobbleheads (Stretch) → v2.0 "The Gallery" launch with session recall + shareable snippet as the distribution loop. Post-launch: v2.x continuous director + persona model improvements. v3.0: deliberately user-driven — direction defined by what 2.0 users ask for. Deferred off the critical path: Voice / TTS, MP4 clip share, Pack Lab UI, Live Moments, overlay mode, personal-context memory. Don't pull work forward across release boundaries without confirming scope.
