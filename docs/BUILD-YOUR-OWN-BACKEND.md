...
`/api/personas` for the extension itself — it's only used by the internal
test scripts (`scripts/test-personas.ts`). The legacy `/watch` reference
web app that used to call it was retired in the v1.5 legacy cleanup.
