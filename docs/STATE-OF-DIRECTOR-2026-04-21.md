...
- **Unstable-tail heuristic** (#80, SET-7) — v3 prompt now distinguishes stable-for-≥2-ticks transcript from chars that just arrived this tick. Router prefers SILENT when the only reactable signal lives in the tail. Threaded through main v3 Haiku + both Cerebras/Groq v3-prompt shadows. Canary analyzer gained a tail-share × silent-rate histogram.
