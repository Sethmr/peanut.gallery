...
- **Howard pack — Jackie ("The Joke Man") v1.8.1** ([`lib/packs/howard/prompts/jackie-martling.ts`](lib/packs/howard/prompts/jackie-martling.ts), [`lib/packs/howard/personas.ts`](lib/packs/howard/personas.ts)). Reference grew from ~25KB / 252 lines to ~80KB / 1165 lines. Adds 10 named tactical comedic moves (`qa_wordplay` / `bar_structure_shaggy` / `marriage_oneliner` / `doctor_health_aging` / `animal_absurd` / `stupid_guy_absurd` / `bathroom_function` / `mid_word_laugh_break` / `interjection_micro` (≤7-word Stern-note default in multi-speaker contexts) / `self_deprecation_pivot`); 4 named registers with explicit 85/10/5/0 trigger distribution (`joke_fire_default` 85% / `heckle_or_plug` 10% / `self_deprecation_pass` 5% / `earnest_acknowledge_then_joke` rare); §10 production fence with formal hard-excludes for the open-source / Europe-deployable surface (no slurs, no rape jokes, no ethnicity-as-punchline, no disability-as-punchline, no age-degradation, no "Polish water skiing" James Byrd joke); expanded family red lines (brothers Bobby + Jimmy 1993 suicide, ex-wife Nancy, current girlfriend Barbara, parents); Stern-question two-frame protocol (acknowledge genius → acknowledge grievance → philosophical pivot → close with joke); Twitter prosody mode for text-output contexts; expanded verbatim joke vault (70+ canonical jokes vs. 26 in v1.8) all screened against §10; expanded relational cast (Barbara, Willie Nelson, Rodney Dangerfield, Peter Bales, Richie Minervini, Rick Dees as the actual coiner of "The Joke Man" — NOT Howard, Jackie Mason, Gershon Legman, Penn Jillette); 23 identity-anchor anecdotes; the cheap-and-generous paradox; 8-question persona self-test before shipping output.

- **Howard pack — Fred ("Earth Dog Fred") v1.8.1** ([`lib/packs/howard/prompts/fred-norris.ts`](lib/packs/howard/prompts/fred-norris.ts), [`lib/packs/howard/personas.ts`](lib/packs/howard/personas.ts)). Reference grew from ~30KB / 553 lines to ~82KB / 1366 lines. Five-mode output spec (A SFX / B zinger / C impression / D silence / E mini-riff) preserved exactly. Adds 6 named B-mode sentence shapes (`two_noun_deflation` / `single_past_tense_indicative` / `numeric_correction` / `single_concession_then_diminish` / `proper_noun_as_deflation` / `robin_direct_address_inside_nixon` — only inside the impression, never outside); 12 named trolly-edge target categories (`pundit_emoting` / `politician_pageantry` / `hype_persona` / `tech_ceo_visionary` / `sports_broadcaster_oversell` / `celebrity_apology_video` / `awards_show_emotion` / `reality_tv_setup` / `wellness_influencer` / `crypto_finance_hype` / `true_crime_narration` / `late_night_oversell`); 13 voice-tells (the §5B non-impression interjections — `uh.` / `right.` / `okay.` / `yeah.` / `no.` / `sure.` / `hmm.` / `[breath]` / `[sigh]` / `anyway.` / `that's it.` / `move on.` / `sounds about right.` — operationally distinct from C-mode impressions, with per-30-turn-session quotas, NEVER laugh in Fred's own voice); 20 Appendix-B director-signal tags so the v3 router can pass register hints; 80+ scenario worked examples; 8 multi-turn dialog scenes; 20 negative examples with diagnosis + correction; volume-and-color rules ("fucking" only when the noun is doing real work, the Nov 1, 2022 Springsteen line as the model).

- **Howard pack — The Troll v1.8.1** ([`lib/packs/howard/prompts/the-troll.ts`](lib/packs/howard/prompts/the-troll.ts), [`lib/packs/howard/personas.ts`](lib/packs/howard/personas.ts)). Targeted enrichment closing the v1.8-flagged 85% → ~95% encyclopedia gap. Adds §1.4 negative-findings list — explicit "do NOT fabricate Stuttering John landmines as historical canon" exclusion covering Gilbert Gottfried (peer rivalry, NOT a target-of-the-bit pairing) and 22 other named celebrities (Donny Osmond, MC Hammer, Oliver Stone, Quincy Jones, Ray Charles, George Takei, Harvey Keitel, Jerry Seinfeld, Yoko Ono, Tiny Tim, Howard Cosell, Muhammad Ali, Mike Tyson, Rudy Giuliani, Mayor Ed Koch, Ed McMahon, David Duke, Paul Shaffer, Joey Buttafuoco, Kathie Lee Gifford, Dolly Parton, Barry White). Anti-fabrication rule added to the kernel — high-leverage anti-hallucination guardrail. The 7-sub-voice composite kernel (Janks / Stuttering John / High Pitch Erik / Beetlejuice / Eric the Actor / Hank / Sal & Richard) and 8 Stuttering John question templates and Howard Stern Anatomy formula and 5-beat prank anatomy and Stern-doctrine red lines preserved exactly.

- **Howard pack — Baba Booey ("The Heckler") v1.8.1** ([`lib/packs/howard/prompts/baba-booey.ts`](lib/packs/howard/prompts/baba-booey.ts), [`lib/packs/howard/personas.ts`](lib/packs/howard/personas.ts)). Reference grew from ~51KB / 427 lines to ~73KB / 1129 lines. Kernel preserved verbatim — the trolly-EP register + 4-tier fact-check-layer patch (CONFIRMS / CONTRADICTS / COMPLICATES / THIN). Adds Section A 40+ landmark quotes by named register (earnest-producer / flustered-butt-of-joke / genuine-laugh / sports-nerd / trolly-heckler / defensive-pushback / self-deprecating / memoir-promotion / mother-Ellen sacred-ground / brother-Steven sacred-ground / Howard's Nov 10 2020 smackdown-foil); Sections B-Q sixteen specific source corpora with dated quotes preserved (SI.com 2009, LAist 2010, TheBPlot 2010, Billboard 2014/2016, Adelphi 2009, Hollywood Reporter ETAF 2025, Greenwich Magazine 2021, Greenwich Parks & Rec, Sound & Vision Gadget Gary 2007-2013, Twitter @robertAbooey 2009-2026); Section R 10 specific moment full-exchange anchors (Baba Booey origin July 26 1990, Maury-from-Brooklyn OJ Bronco chase June 17 1994, Artie-Gary fight Feb 5 2009, Mets first pitch May 9 2009, Bawf Jan 7 2019, NYSE Dec 5 2022, 57 Booey Blunders Mar 14 2018, bagel alerts Mar 25 2024, Tom Brady Roast May 6 2024, ETAF May 8 2025); Part IV corpus-integrity notes flagging Marksfriggin paraphrase risk + the "I play a character to some degree, but I don't bring that home with me" operating-philosophy calibration anchor.

- **TWiST pack — Lon ("The Reframe") v1.8.1** ([`lib/packs/twist/prompts/lon-harris.ts`](lib/packs/twist/prompts/lon-harris.ts), [`lib/packs/twist/personas.ts`](lib/packs/twist/personas.ts)). Reference grew from ~46KB / 545 lines to ~76KB / 732 lines. Kernel preserved verbatim. Adds 6 named voice modes (`considered_reframe` (default — concessive opener → structural pivot → landed observation) / `dry_deflation` (hype claim → flat translation, "which kind of sounds a lot like…") / `film_compression` (filmography chain or genre-label coinage as compression — "Red Notice genre," "tech cinematic universe") / `sincere_enthusiasm` (rare flash on craft — "It's just awesome") / `direct_address` (advice-as-attack on a named subject — Hollywood-corporate-strategy only for fake-monologue variant) / `recursive_meta` (writer noticing the writing — "this sentence you're reading right now")); cross-cutting `jason_softening` register with formalized Pattern A-E abstract (minimal-concession, useful-version-restatement, genre-name reframe, numbers-into-story translation, "I'd defer to Alex" lane-discipline) — symmetric to Molly's "no-characterize-the-2023-departure" discipline; 10 Deployment Rules (compressed in kernel, canonical in reference); frequency-coded vocabulary fingerprint (HIGH/MED/LOW openers, hedges, closers, references, phrase-level tells); massively expanded verbatim exemplar bank organized by mode, dated and sourced from the 1997-2026 corpus.

- **TWiST pack — Alex ("The Data Comedian") v1.8.1** ([`lib/packs/twist/prompts/alex-wilhelm.ts`](lib/packs/twist/prompts/alex-wilhelm.ts), [`lib/packs/twist/personas.ts`](lib/packs/twist/personas.ts)). Reference grew from ~39KB / 514 lines to ~82KB / 1277 lines. Kernel preserved verbatim. Adds 4 named comedic structures (`number_punchline` (number IS the punchline) / `one_word_deflation` (full number → bolded monosyllable: "What?", "Bonkers.") / `cant_believe_aftermath` (finish a valuation, immediately apologize to the math) / `rule_of_40_grading` (any SaaS name triggers growth + margin + verdict)); 3 named spoken rhythms (`machine_gun_earnings` / `polite_interrupt` / `self_audit_parenthetical`); 4 named emotional registers with explicit switch guidance (`excitable_default` / `earnest_analyst` (real money / real risk — "frankly" + no italics tell) / `sincere_nerd_out` (genuine joy at a beautifully-constructed business model — GitLab / Claude Code / Snowflake operating leverage / Anthropic 600% growth) / `warm_self_deprecation`); era-by-era voice evolution (TNW 2009-2013 → TC1 2013-2016 → Mattermark 2016-2017 → CB News 2017-2019 → TC2 + The Exchange 2019-2024 → CO + TWiST 2024-present); Cautious Optimism column-convention catalog (canonical opener, day-tag variants, sign-off register selector, Trending Up/Down structural template, section transitions); cast deep cuts with per-peer verbatim including the Chris Gates warmth rule (default to genuine warmth, no irony) and the Octavia-only-named-publicly rule (the other two children NEVER named); 5 compact in-character templates; light political guardrails preserved verbatim.

- **TWiST pack — Molly ("The Journalist") v1.8.1** ([`lib/packs/twist/prompts/molly-wood.ts`](lib/packs/twist/prompts/molly-wood.ts), [`lib/packs/twist/personas.ts`](lib/packs/twist/personas.ts)). Reference grew from ~59KB / 592 lines to ~119KB / 1025 lines. Kernel preserved verbatim — NPR-journalist register + 4-tier fact-check-layer patch (CONFIRMS / CONTRADICTS / COMPLICATES / THIN with canonical lines in her reporter-desk voice). Adds §2A CNET / Buzz Out Loud era voice-formation material (the consumer-tech-host years that built her audience and remain >50% of her professional life — register to reach for when the Director cues her on a non-climate topic); §11A EITP 2026 verbatim corpus (Eps 128, 131, 132 — full cold-open transcripts, the "if you aren't a grid asset are we even friends" 2026 distillation, the rapport-token cluster, the fact-checker pivot architecture, the translation move in action, the Discord-bar metaphor 2026 outro update); §11B Marketplace Tech 2019-2021 verbatim corpus (the NPR-host foundation voice, the "Marketplace Tech, I'm Molly Wood" five-word locked sign-in); §12A the translation move formally articulated as her core on-mic skill (compression-token + interrogative paraphrase + softener + hand-back, with three named failure modes — putting-words-in-mouth / no-jargon-strip / sounds-smug); §21A Peanut Gallery reaction snippets (Twist-pack tuned, organized by 16 trigger categories — pre-baked retrieval material for the live-commentary use case); §23 fast-reference appendix — the 10 things a persona must always do (operational invariants matching Lon's 10 deployment rules pattern); expanded §4 catchphrases from 25 to 40.

- **TWiST pack — Jason ("The Host") v1.8.1 — load-bearing, double-size** ([`lib/packs/twist/prompts/jason-calacanis.ts`](lib/packs/twist/prompts/jason-calacanis.ts), [`lib/packs/twist/personas.ts`](lib/packs/twist/personas.ts)). Reference grew from ~62KB / 804 lines to ~110KB / 1309 lines. SKILL.md kernel preserved verbatim. Saved for last per Seth's "double the size of anyone else" direction — this corpus drew from two companion source files (197KB Full + 85KB Consolidated = 282KB combined). Adds the pre-magazine technical layer (1985-1993 PCjr / Atari / soldered-memory-chips backstory); the Cyber Surfer / Jerry Colonna / Flatiron 1995 origin (Jason's actual VC-adjacent debut is **15 years earlier** than the conventional Sequoia-Scout-2009 story); the Calm pricing-coaching anecdote (the cleanest founder-coaching story on record — triangulate against offline alternative + cadence + elasticity); the 2024-2026 expanded corpus (full Replicants / OpenClaw / Great Hiring Hiatus passages with anaphora pattern preserved, the bimodal "two types of employees" framing — DO NOT soften to three, the OpenClaw / Anthropic critique, the Sam Altman warning extended with Borg / Zuckerberg-school-of-business framing, the "rule #1: wait 72 hours" axiom, structural facts for 2026, vocabulary additions: Replicants / OpenClaw / Clawdbot / The Great Hiring Hiatus / Trump Truth Tantrums / donnybrook / the double nickel / Mayor McChe / Gen-Xi / Gentleman's RIF / "the Wrath of Khan is over" / "the age of autonomy is here"); the self-positioning vs. other VCs contrast bank (Sequoia respect-without-imitation / a16z unredacted "soulless / industrialized / factorized" — DO NOT SOFTEN / Y Combinator rivalrous-with-respect / Founders Fund cordial but distant / Benchmark Gurley-positive / Ron Conway as spiritual predecessor / Naval mourned friendship-loss / anti-types — pay-to-pitch / solo capitalists / hedge-fund tourists / thesis-tweeters); the Howard Stern playbook confession (Stern as persona-engineering source, McLaughlin as format source, Charlie Rose as interview-technique source — the Sultan-of-Science character was deliberately engineered by Jason as moderator, NOT fan-coined); the Founder University coaching corpus (multi-tier-pricing diagnostic, three-axis AI-incumbent challenge — Bloomberg / ChatGPT / "what's yours?", world-class-design diagnostic, product-velocity diagnostic, substance-then-delivery coaching order, "get to work" two-word imperative close); the persona deployment guide (default mode mix, length calibration, Brooklyn-marker density, roll-call discipline, do-not-fabricate list, asymmetric political guardrails, emotional-truth guardrails — Jason gets choked up about three things only: his father, his mother retiring, his portfolio founders winning, drift-recovery moves); the **three-test final pass** — Tim Ferriss test (would Tim recognize this as the J Cal he interviewed?) / Sacks test (does it have a position to push against?) / Founder test (would a founder come away with a concrete next action?) — pass all three or revise.

### Notes for the merge

- **Content-only release.** No code changes, no API surface change, no architecture change. Every kernel the model reads at fire time is byte-identical to v2.0.1. What changed is the per-persona reference dossier the kernel pattern-matches against — invisible to the user except via voice fidelity.
- **All scaffolding flags preserved.** Baba and Molly retain `producerMode: "layered-fact-checker"`. The other six personas have no producer-specific scaffolding (content-only kernel upgrades).
- **Director-side wiring updated** for all 8 personas — `directorHint` now enumerates each persona's named tactical-move taxonomy so the v3 router can theoretically bias mode selection per turn. Today the kernel self-selects when no hint is present; the wiring is in place for when the router gets taught register hints.
- **All 26 director fixtures still green at 50 runs each** plus director-v2 unit tests, semantic-cache, claim-detector, highlight-picker, subscription-store, stripe-webhook all passing.
- **Marketing site repo doesn't ship a paired update** — this is a backend-only voice-fidelity release. The user-facing surfaces (extension UI, marketing copy, ToS, Privacy) are unchanged.
