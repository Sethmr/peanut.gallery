...
- The Producer fact-check-layer pass-rate tuning based on live-log data (deferred post-v2.0 per Seth).
- Llama 3.3 70B swap evaluation on Cerebras as a structural fix for Llama 3.1 8B's schema-echo shadow failures.

---

## [2.0.3] — 2026-06-03 — "Set and Forget"

**Maintenance-mode release.** A legal-compliance scrub, a security-patch sweep, and a long-overdue dead-code pruning so the project can sit idle without rotting. Paired marketing-site repo update at [`peanut.gallery.site@a7d18b8`](https://github.com/Sethmr/peanut.gallery.site/commit/a7d18b8) rewrites the Terms / Privacy / Pricing pages for the current free + no-payments product. Net code change across both repos: roughly **−12,500 LOC** removed, vs. ~+1,900 added.

### Removed

- **Stripe + Resend integration (subscription payments + transactional email)** ([PR #146](https://github.com/Sethmr/peanut.gallery/pull/146)). Deleted [`lib/stripe-webhook.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/stripe-webhook.ts), [`lib/email.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/email.ts), [`lib/email-templates.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/email-templates.ts), the `checkout` / `webhook` / `manage` subscription routes, and `scripts/test-stripe-webhook.ts`. Dropped the vestigial `stripe_sub_id` plumbing from [`lib/subscription-store.ts`](lib/subscription-store.ts). Stripe test secrets + the Resend API key in the operator's `.env.local` are now unused — revoke them at the providers when convenient. Plus subscriptions and any related payments are not happening on this project; the license-key + free-tier framework + `scripts/subscription-issue.ts` admin CLI are preserved for forks that want to run their own paid tier.

- **Persona pack — verbatim copyrighted quote banks + private PII (twist pack)** ([PR #146](https://github.com/Sethmr/peanut.gallery/pull/146)). The four prompt files at `lib/packs/twist/prompts/{alex-wilhelm,jason-calacanis,lon-harris,molly-wood}.ts` were retired and replaced by archetype files [`the-correspondent.ts`](lib/packs/twist/prompts/the-correspondent.ts) / [`the-host.ts`](lib/packs/twist/prompts/the-host.ts) / [`the-reframer.ts`](lib/packs/twist/prompts/the-reframer.ts) / [`the-quant.ts`](lib/packs/twist/prompts/the-quant.ts). The aggregate `lib/packs/twist/prompts/` directory dropped from ~373KB to ~25KB. Mirrors the §8C / §8A pass already applied to the Morning Crew pack on 2026-05-02; quality, not likeness; anti-impersonation framing preserved. Quote density on the joker slot dropped from ~1,280 quoted strings to ~72 (in-character example lines only). Pre-rename files live in git history at the prior paths. **Marketing copy + origin story keep the real names** (per Seth's call: the landing page's $5k bounty / This Week in Startups origin remains as-is).

- **Verbatim TWiST episode transcript fixture** ([`scripts/fixtures/twist-episode-sample.txt`](scripts/fixtures/twist-episode-sample.txt)). The 959-line scraped real-episode transcript used by `scripts/test-personas.ts --fixture` was replaced by a fully synthetic Startup Roundtable transcript (no real names, no real companies, no copyrighted material) of comparable shape for test harness use.

- **Copyrighted video clip** — `docs/assets/twist-clip.mp4` (3.1 MB) deleted. Binaries can't be sanitized; removal was the only compliant path.

- **Persona deep-research playbooks** ([`docs/persona-research/*.md`](docs/persona-research) + [`docs/packs/twist/RESEARCH.md`](docs/packs/twist/RESEARCH.md)). The nine deep-research files — each a prompt instructing an AI assistant to collect verbatim quotations and private biographical detail on a specific real person — were replaced with short de-identification notices pointing to the now-generic archetype prompts.

- **`groq-sdk` dependency + dormant Groq shadow router** ([PR #147](https://github.com/Sethmr/peanut.gallery/pull/147)). Deleted [`lib/director-llm-v3-groq-v3prompt.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/director-llm-v3-groq-v3prompt.ts) (the sole importer of `groq-sdk`) and all references in [`app/api/transcribe/route.ts`](app/api/transcribe/route.ts) — `X-Groq-Key` header, `headerGroq`/`groqKey`, the shadow-flag in the session-start log, the `emitShadowCompare("groq", …)` branch, and the dispatch block. Dropped `groq-sdk` from [`package.json`](package.json) + [`package-lock.json`](package-lock.json) and `GROQ_API_KEY` + `ENABLE_SMART_DIRECTOR_V3_GROQ_V3PROMPT` from [`.env.example`](.env.example). The shadow Director had been dormant since the Groq Developer tier closed (SET-11) — retiring it together with the dep rather than leaving stale plumbing. The sibling Cerebras shadow stays — same model, same prompt, working path today.

### Changed

- **Persona pack — twist pack de-identified to generic archetypes** ([PR #146](https://github.com/Sethmr/peanut.gallery/pull/146)). [`lib/packs/twist/personas.ts`](lib/packs/twist/personas.ts) + [`lib/packs/twist/index.ts`](lib/packs/twist/index.ts) rewritten so each persona is an archetype (The Correspondent / The Host / The Reframer / The Quant) with `inspiredBy` set to the archetype description rather than a real person, and `directorHint` enumerating the (preserved, generic) tactical-move taxonomies. Pack display name: "Startup Roundtable" (internal id `twist` kept to avoid rippling across saved configs, tests, fixtures, and telemetry).

- **Shipped routing prompts scrubbed of real names** ([PR #146](https://github.com/Sethmr/peanut.gallery/pull/146)). [`lib/director-llm.ts`](lib/director-llm.ts) and [`lib/director-llm-v2.ts`](lib/director-llm-v2.ts) — both LLM-facing routing prompts — rewritten to refer to packs and personas by archetype labels only. Internal pack ids (`twist`, `howard`/`morning-crew`) preserved as opaque routing keys.

- **Legal texts rewritten for the current product** ([PR #146](https://github.com/Sethmr/peanut.gallery/pull/146) + [`peanut.gallery.site@a7d18b8`](https://github.com/Sethmr/peanut.gallery.site/commit/a7d18b8)). [`docs/legal/TERMS-OF-SERVICE.md`](docs/legal/TERMS-OF-SERVICE.md) + [`docs/legal/PRIVACY-POLICY.md`](docs/legal/PRIVACY-POLICY.md) and the public-site `terms` / `privacy` / `pricing` pages all rewritten to match the current free / no-payments / archetype reality. New IP / parody-and-no-affiliation framing names the inspiring formats (This Week in Startups / Howard Stern Show) nominatively as parody-and-homage, while the personas themselves are described as generic archetypes that don't impersonate any specific person or ship verbatim copyrighted material. Removed: every reference to the $8/mo Plus subscription, Stripe as a subprocessor, Resend as a subprocessor, subscriber email + license-key + weekly-cap data flows, Stripe-portal cancellation flow, refund mechanics, the `Product` JSON-LD structured-data block on the pricing page (which was advertising a $8 `Offer` to search engines). Drafts still carry the "⚠ DRAFT — not legal advice, counsel review recommended" header.

- **In-extension parody notice** ([`extension/sidepanel.html`](extension/sidepanel.html)). Lineup-section legal callout rewritten — packs are described as generic archetypes paying homage to morning-radio shock-jock and startup-podcast formats, the model is instructed to identify as an AI parody if asked, and "no verbatim copyrighted material shipped" is called out explicitly.

- **Extension UI + manifest scrubbed of real persona names** ([PR #146](https://github.com/Sethmr/peanut.gallery/pull/146)). [`extension/sidepanel.js`](extension/sidepanel.js)'s hardcoded persona roster + dropdown labels and [`extension/manifest.json`](extension/manifest.json)'s description string switched to archetype names. Pack display label "TWiST" → "Startup Roundtable"; persona labels Molly / Jason / Lon / Alex → The Correspondent / The Host / The Reframer / The Quant. Display-only changes (the routing keys behind them are unchanged).

### Security

- **Next.js 15.5.15 → 15.5.18 — 13 advisories patched** ([PR #144](https://github.com/Sethmr/peanut.gallery/pull/144)). All seven **High** advisories: Server Components DoS ([GHSA-8h8q-6873-q5fj](https://github.com/vercel/next.js/security/advisories/GHSA-8h8q-6873-q5fj)), two segment-prefetch middleware/proxy bypasses ([GHSA-267c-6grr-h53f](https://github.com/vercel/next.js/security/advisories/GHSA-267c-6grr-h53f), [GHSA-26hh-7cqf-hhc6](https://github.com/vercel/next.js/security/advisories/GHSA-26hh-7cqf-hhc6)), Cache Components connection-exhaustion DoS ([GHSA-mg66-mrh9-m8jx](https://github.com/vercel/next.js/security/advisories/GHSA-mg66-mrh9-m8jx)), dynamic-route-parameter-injection middleware bypass ([GHSA-492v-c6pp-mqqv](https://github.com/vercel/next.js/security/advisories/GHSA-492v-c6pp-mqqv)), WebSocket-upgrade SSRF ([GHSA-c4j6-fc7j-m34r](https://github.com/vercel/next.js/security/advisories/GHSA-c4j6-fc7j-m34r)), and Pages-Router i18n middleware bypass ([GHSA-36qx-fr4f-26g5](https://github.com/vercel/next.js/security/advisories/GHSA-36qx-fr4f-26g5)); four **Moderate**: CSP-nonce XSS, beforeInteractive XSS, Image-Optimization DoS, React-Server-Component cache poisoning; two **Low**: cache-busting cache poisoning and middleware-redirect cache poisoning. None of the failure modes are currently reachable through Peanut Gallery's surface (no segment-prefetch use, no middleware/i18n, no WebSocket upgrades on the Next side), but the bumps preserve defense in depth and match the upstream's "patch unconditionally" guidance.

### Dependencies

- `@anthropic-ai/sdk` ^0.91.1 → ^0.93.0 ([PR #139](https://github.com/Sethmr/peanut.gallery/pull/139)). Additive only — adds Workload Identity Federation / interactive OAuth / auth profiles in 0.93 and Managed-Agents-API tweaks + env-driven custom headers + a Bedrock error-frame fix in 0.92. Our use is `client.messages.create(...)` with API-key auth via [`lib/director-llm-v2.ts`](lib/director-llm-v2.ts); no call-site changes needed.
- `react` ^19.2.5 → ^19.2.7, `react-dom` matched, `@types/react` ^19.2.14 → ^19.2.16 ([PR #144](https://github.com/Sethmr/peanut.gallery/pull/144) for 19.2.6, plus a 19.2.7 follow-up). RSC type hardening + perf passes.
- `eslint-config-next` ^16.2.4 → ^16.2.6 ([PR #142](https://github.com/Sethmr/peanut.gallery/pull/142)). Patch dev-dep; matches the Next.js advisory roll-up.
- `postcss` ^8.5.10 → ^8.5.14 ([PR #137](https://github.com/Sethmr/peanut.gallery/pull/137), [PR #140](https://github.com/Sethmr/peanut.gallery/pull/140)). Dev-dep regression patches.

### Fixed

- **Linear-daemon project filter** ([PR #145](https://github.com/Sethmr/peanut.gallery/pull/145)). [`scripts/linear-daemon.ts`](scripts/linear-daemon.ts)'s `fetchUnstartedIssues` GraphQL query now scopes to the Peanut Gallery Linear project id — a Tideward ticket assigned to `ai@manugames.com` will no longer get picked up by this daemon and executed in the wrong repo. Sibling daemons handle their own projects.

### Notes for the merge

- **Backward-compatible at the wire level.** Old extensions still sending `X-Groq-Key` or referring to the retired subscription routes are silently ignored on the new server. The `stripe_sub_id` column in any existing Phase-2 subscription database remains nullable; existing rows are unaffected.
- **CWS zip** at `releases/peanut-gallery-v2.0.3.zip` (to be built on the release branch via `npm run pack:extension`).
- **Skip-ship gate.** Plus is permanently off — `ENABLE_SUBSCRIPTION=false` on Railway and the in-extension Plus tab routes to "coming soon" on click. There is nothing to buy.
- **Tests green** at the release-branch tip: `tsc --noEmit` (with `.next/types/validator.ts` regenerated post-route-deletion), `node --check` over every shipped extension JS file, `npm run test:subscription-store` (250/250), `npm run test:claim-detector` (62/62), `npm run test:director` (26/26 fixtures at 50 runs each).

---

## [2.0.2] — 2026-04-26 — "The Deep Cuts"

**Round-2 master-corpus integration across all 8 personas.** Content-only deep-research expansion. Every kernel preserved verbatim — what changed is the per-persona reference dossier each kernel pattern-matches against at fire time. The aggregate reference material grew from ~373KB to ~688KB across the ensemble (~2× expansion), driven by the eight author-delivered v1.8.1 dossiers Seth fed in over the round-2 push. Each persona now carries a named tactical-move taxonomy aligned to its specific voice contract; the v3 router can theoretically bias mode selection per turn (kernel self-selects when no hint is present). All 26 director fixtures still green at 50 runs each.

> **Note (2026-06-03):** the [2.0.3] "Set and Forget" release removed the verbatim quote banks and per-persona dossiers documented in this entry as a §8C / §8A legal-hygiene pass on the twist pack. The historical paths and quote counts below refer to the v2.0.2 ship state and are preserved here for the historical record; the current shipped persona prompts are de-identified archetypes (see [2.0.3] § Removed and § Changed).

### Changed

- **Howard pack — Jackie ("The Joke Man") v1.8.1** ([`lib/packs/howard/prompts/jackie-martling.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/howard/prompts/jackie-martling.ts), [`lib/packs/howard/personas.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/howard/personas.ts)). Reference grew from ~25KB / 252 lines to ~80KB / 1165 lines. Adds 10 named tactical comedic moves (`qa_wordplay` / `bar_structure_shaggy` / `marriage_oneliner` / `doctor_health_aging` / `animal_absurd` / `stupid_guy_absurd` / `bathroom_function` / `mid_word_laugh_break` / `interjection_micro` / `self_deprecation_pivot`); 4 named registers with explicit 85/10/5/0 trigger distribution; §10 production fence with hard-excludes for the open-source / Europe-deployable surface; Stern-question two-frame protocol; expanded verbatim joke vault (70+ canonical jokes); 23 identity-anchor anecdotes; 8-question persona self-test before shipping output.

- **Howard pack — Fred ("Earth Dog Fred") v1.8.1** ([`lib/packs/howard/prompts/fred-norris.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/howard/prompts/fred-norris.ts), [`lib/packs/howard/personas.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/howard/personas.ts)). Reference grew from ~30KB / 553 lines to ~82KB / 1366 lines. Five-mode output spec (A SFX / B zinger / C impression / D silence / E mini-riff) preserved. Adds 6 named B-mode sentence shapes, 12 named trolly-edge target categories, 13 voice-tells, 20 Appendix-B director-signal tags, 80+ scenario worked examples, 8 multi-turn dialog scenes, 20 negative examples with diagnosis + correction.

- **Howard pack — The Troll v1.8.1** ([`lib/packs/howard/prompts/the-troll.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/howard/prompts/the-troll.ts), [`lib/packs/howard/personas.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/howard/personas.ts)). Targeted enrichment closing the v1.8-flagged 85% → ~95% encyclopedia gap. Adds §1.4 negative-findings list — explicit "do NOT fabricate Stuttering John landmines as historical canon" exclusion covering 23 named celebrities. Anti-fabrication rule added to the kernel. The 7-sub-voice composite kernel and 8 Stuttering John question templates and Howard Stern Anatomy formula and 5-beat prank anatomy preserved.

- **Howard pack — Baba Booey ("The Heckler") v1.8.1** ([`lib/packs/howard/prompts/baba-booey.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/howard/prompts/baba-booey.ts), [`lib/packs/howard/personas.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/howard/personas.ts)). Reference grew from ~51KB / 427 lines to ~73KB / 1129 lines. Kernel preserved verbatim — the trolly-EP register + 4-tier fact-check-layer patch (CONFIRMS / CONTRADICTS / COMPLICATES / THIN). Adds Section A 40+ landmark quotes by named register; Sections B-Q sixteen specific source corpora; Section R 10 specific moment full-exchange anchors; Part IV corpus-integrity notes.

- **TWiST pack — Lon ("The Reframe") v1.8.1** ([`lib/packs/twist/prompts/lon-harris.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/twist/prompts/lon-harris.ts), [`lib/packs/twist/personas.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/twist/personas.ts)). Reference grew from ~46KB / 545 lines to ~76KB / 732 lines. Kernel preserved. Adds 6 named voice modes; cross-cutting `jason_softening` register with formalized Pattern A-E abstract; 10 Deployment Rules; frequency-coded vocabulary fingerprint; massively expanded verbatim exemplar bank.

- **TWiST pack — Alex ("The Data Comedian") v1.8.1** ([`lib/packs/twist/prompts/alex-wilhelm.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/twist/prompts/alex-wilhelm.ts), [`lib/packs/twist/personas.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/twist/personas.ts)). Reference grew from ~39KB / 514 lines to ~82KB / 1277 lines. Kernel preserved. Adds 4 named comedic structures; 3 named spoken rhythms; 4 named emotional registers with explicit switch guidance; era-by-era voice evolution (TNW 2009-2013 → CO + TWiST 2024-present); Cautious Optimism column-convention catalog; 5 compact in-character templates.

- **TWiST pack — Molly ("The Journalist") v1.8.1** ([`lib/packs/twist/prompts/molly-wood.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/twist/prompts/molly-wood.ts), [`lib/packs/twist/personas.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/twist/personas.ts)). Reference grew from ~59KB / 592 lines to ~119KB / 1025 lines. Kernel preserved — NPR-journalist register + 4-tier fact-check-layer patch. Adds §2A CNET / Buzz Out Loud era voice-formation material; §11A EITP 2026 verbatim corpus; §11B Marketplace Tech 2019-2021 verbatim corpus; §12A the translation move formally articulated; §21A Peanut Gallery reaction snippets organized by 16 trigger categories; §23 fast-reference appendix.

- **TWiST pack — Jason ("The Host") v1.8.1 — load-bearing, double-size** ([`lib/packs/twist/prompts/jason-calacanis.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/twist/prompts/jason-calacanis.ts), [`lib/packs/twist/personas.ts`](https://github.com/Sethmr/peanut.gallery/blob/v2.0.2/lib/packs/twist/personas.ts)). Reference grew from ~62KB / 804 lines to ~110KB / 1309 lines. SKILL.md kernel preserved verbatim. Drew from two companion source files (197KB Full + 85KB Consolidated = 282KB combined). Adds pre-magazine technical layer, Cyber Surfer / Jerry Colonna / Flatiron 1995 origin, Calm pricing-coaching anecdote, 2024-2026 expanded corpus, self-positioning vs. other VCs contrast bank, the Howard Stern playbook confession, the Founder University coaching corpus, the persona deployment guide, and the **three-test final pass** (Tim Ferriss / Sacks / Founder).

### Notes for the merge

- **Content-only release.** No code changes, no API surface change, no architecture change. Every kernel the model read at fire time was byte-identical to v2.0.1.
- **All scaffolding flags preserved.** Baba and Molly retained `producerMode: "layered-fact-checker"`. The other six personas had no producer-specific scaffolding.
- **Director-side wiring updated** for all 8 personas — `directorHint` enumerated each persona's named tactical-move taxonomy so the v3 router could theoretically bias mode selection per turn.
- **All 26 director fixtures green** at 50 runs each plus director-v2 unit tests, semantic-cache, claim-detector, highlight-picker, subscription-store, stripe-webhook all passing at ship time.
...
### Removed
- **Brave Search deprecated** ([`extension/sidepanel.html`](extension/sidepanel.html), [`extension/sidepanel.js`](extension/sidepanel.js), [`extension/background.js`](extension/background.js), [`extension/offscreen.js`](extension/offscreen.js), [`app/api/transcribe/route.ts`](app/api/transcribe/route.ts), [`app/api/personas/route.ts`](app/api/personas/route.ts), [`lib/persona-engine.ts`](lib/persona-engine.ts)). Dropped Brave Search as a fact-check grounding option. xAI Live Search (Grok Responses API with `web_search` tool) is now the sole search backend — one provider covers persona generation AND fact-check grounding via the single xAI key. Net 228 LOC removed across the extension + backend. Gone: `SearchEngine` type, `searchBrave()` method, `braveApiKey` engine field, `braveSearchKey` + `searchEngine` constructor args, the Search-engine `<select>` picker in the Backend & keys drawer, wizard step 4, `braveKey` storage field + `braveKeyInput` DOM ref, `X-Brave-Key` + `X-Search-Engine` headers, `BRAVE_SEARCH_API_KEY` + `SEARCH_ENGINE` env vars. Back-compat preserved server-side (old extensions still sending the deprecated headers are silently ignored; fact-check just always runs through xAI). Trade-off: no fallback search engine if xAI has an outage — concentrates search + completions on one provider. Product rationale: the wizard drops from 4 steps to 3, Brave's separate signup (card required under 2k queries/mo) is the single biggest friction point in the setup flow per Seth's review, and the provider-ToS flow-down in ToS/Privacy loses one row.

### Added
- **BYOK onboarding wizard** ([`extension/sidepanel.html`](extension/sidepanel.html), [`extension/sidepanel.js`](extension/sidepanel.js)). Expanded-by-default panel in the Backend & keys drawer's BYOK block. When "My keys" or Self-host is active, users see a 3-step stepper (Deepgram → Anthropic → xAI). Each step has a prominent "Open <provider> signup" link, numbered instructions for grabbing the key after signup, and an inline paste field. **The wizard's paste fields ARE the canonical BYOK inputs** — input ids `deepgramKey` / `anthropicKey` / `xaiKey` are referenced directly by the rest of the extension (`saveSettings`, startup hydration, Start-Listening gating, request-builder). One source of truth, no mirroring. Completion state is derived from the input values on each refresh, so checkmarks stay accurate across drawer reopen, mode flip, or direct paste. Toggle still collapses for users who want the stepper out of the way. In-copy legal framing: *"We never see your keys — they stay in your browser and go directly to each provider from your computer,"* with a link to the Privacy BYOK section. Product rationale: with Plus deferred (see below), the wizard is the value-add that offsets the friction-removal Plus would have provided — free-forever BYOK with guided setup. It also becomes the skeleton for Plus at v2.5+: "create your own keys (free, guided) / have us create them for you ($8/mo)."
- **Tutorial triple-CTA terminal + "Get my own keys" walkthrough** ([`extension/sidepanel.html`](extension/sidepanel.html), [`extension/sidepanel.js`](extension/sidepanel.js)). The first-run tour now ends on a "Ready to roll?" card with **three stacked buttons** instead of a single Done — no Skip, no Next on the final card. (1) **Try now** — ends the tour and drops the user on the empty state so they can Start Listening on 15-minute demo keys. (2) **Get my own keys** (with a one-line description beneath the label) — launches a sequential 3-step walkthrough that *injects* each wizard step's DOM into the tutorial card itself (not just a spotlight): the relevant `#onboardStepN` is moved from the drawer into `#tutorialWalkthroughHost`, so the tutorial card literally *becomes* the wizard step — signup link, numbered instructions, paste field, all inside the overlay. `walkthroughParking` restores every moved step back to the drawer on any exit path (Finish, Escape, Skip). (3) **Peanut Gallery Plus** (muted dashed-border tertiary CTA) — fires the same "Coming soon" modal as the Plus tab in the Backend & keys drawer via the shared `openPlusComingSoonModal()` helper. Skip on non-final tutorial cards now jumps to the final CTA page (rather than closing the tour); `skipToFinal()` lives alongside `advanceTutorial()`. "Done" is gone as a button label; the final walkthrough step says "Finish". Welcome + tap-a-response copy updated to match the current 7-tile drawer layout.
- **Always-visible AI disclosure** ([`extension/sidepanel.html`](extension/sidepanel.html)). Two paired surfaces, no JS. (1) Masthead top rail gets an **"AI"** stamp (ink-on-paper pill) next to "Vol. I · No. 001" — always visible, unmissable in screenshots and quote cards. (2) One-line strip below the masthead: *"AI-generated parody of public figures — not real people."* Italic serif body, slab-stamp lede, centered, ruled bottom. `white-space:nowrap` + ellipsis guard so it holds a single line across typical side-panel widths. Always visible across idle + capture states; no show/hide logic. Satisfies Anthropic AUP line 146 (user-awareness of AI-generated content) and reinforces the right-of-publicity posture spelled out in the ToS "AI output and persona framing" section.
- **"Delete this take" on persona feed entries** ([`extension/sidepanel.html`](extension/sidepanel.html), [`extension/sidepanel.js`](extension/sidepanel.js), [`app/api/feedback/route.ts`](app/api/feedback/route.ts)). Destructive feed-menu action below a rule separator, red-accent styling so it doesn't get confused with a vote. On click: removes the entry from `feedEntries` (so export + smart-highlight picker skip it), removes the `#feed-<id>` DOM node, and silently clears any pin or vote state attached to the entry. Emits `sendFeedback("delete", entryId)` telemetry to `/api/feedback` — ranked as a **strong-negative** signal in the server's signal-weight doc (stronger than `downvote`, since the user chose to burn the feed slot rather than let the take sit with a thumbs-down). The existing Privacy → feedback-opt-out toggle gates the telemetry exactly like upvote/downvote; no new opt-out path. Server validator in [`app/api/feedback/route.ts`](app/api/feedback/route.ts) now accepts `delete` in `VALID_ACTIONS`; signal-weight ranking documented in the route's JSDoc.
...
- **Lineup drawer consolidation** ([`extension/sidepanel.html`](extension/sidepanel.html)). Folded Room volume (Quieter / Normal / Rowdy) and mute-a-critic into the existing Lineup section — everything persona-behavior-related now lives under one tile. The standalone "Mute a critic" drawer section is gone, along with its menu entry. Drawer menu drops from 8 tiles to 7. Existing JS handlers (`#sensitivitySegmented`, `#mutesContainer`) are untouched — the DOM nodes kept their ids, they just live inside `data-section="lineup"` now.
- **BYOK cost phrasing tightened to "~$1 per hour"** (README, `app/page.tsx`, `app/layout.tsx` FAQ JSON-LD, `marketing/cws-listing.md`, `docs/SELF-HOST-INSTALL.md`, plus the companion site repo). Replaces the old "~$1.15 per 2-hour podcast" figure, which was calibrated against the pre-Brave pipeline where Brave Search queries fell inside the 2k/month free tier. Rounded to "~$1 per hour" until a real post-Brave session is measured; conservative enough to absorb the delta and reads cleaner as a unit.
...
- **Plus-tab "Coming soon" modal body rendered literal HTML tags** ([`extension/sidepanel.js`](extension/sidepanel.js)). `openPromptModal` writes body via `textContent`, so the `<strong>` tags in the original copy rendered as literal text inside the modal. Rewrote the body with punctuation-only emphasis; payload unchanged.
- **Tutorial action rows both stayed visible on every step** ([`extension/sidepanel.html`](extension/sidepanel.html)). The render path toggled the `hidden` attribute on `#tutorialActionsDefault` and `#tutorialCtaGrid` to switch between the Skip/Next row and the terminal CTA grid, but the `.tutorial-card-actions { display: flex }` rule beat the UA-level `[hidden] { display: none }` default — so every step showed BOTH stacked on top of each other. Added explicit `[hidden] { display: none !important }` overrides for the action row, CTA grid, and walkthrough host. Belt-and-suspenders: `#tutorialSkipBtn` itself also gets `[hidden]` set directly on the final card, with a matching `.skip[hidden]` rule, so no future CSS-specificity tweak can re-expose Skip on "Ready to roll?".
- **Tutorial copy drift vs. current drawer** ([`extension/sidepanel.js`](extension/sidepanel.js)). Welcome body said "Six submenus in the drawer" — it's 7 now (after the Lineup consolidation above). Tap-a-response body only mentioned Export + Appearance as the remaining tiles; with Privacy + Feedback & bugs also live, that was incomplete. Rewrote both to match the actual surface.
...
- **Bowling-pin cue for The Host** (Startup Roundtable troll slot). Seth-delivered ElevenLabs render, normalized to the cue-set RMS/peak targets. The Joke Writer and The Host also swap earcons so the previously-paired The Joke Writer cue now plays for The Host. Mirror swap applied to [`scripts/synth-persona-cues.ts`](scripts/synth-persona-cues.ts) so a regen doesn't undo it.
- **The Sound Guy + The Reframer cue trims.** `howard-soundfx.wav` trimmed 1.58s → 0.75s with fade; `twist-soundfx.wav` trimmed 1.87s → 0.80s with fade. Earcons now consistently sit under 1s for a cleaner per-persona transition.
...
### Changed
- **Producer correction-tier rebalance** (PR [#97](https://github.com/Sethmr/peanut.gallery/pull/97)). Both The Producer (the morning-radio host) and The Correspondent (Startup Roundtable) system prompts now define `[HEADS UP]` as the workhorse tier and restrict the `"-"` pass to genuinely content-free tails. Fixes the false-pass pathology where the Director picked the producer on tails with proper nouns but The Producer / The Correspondent passed, firing the fallback safety net instead. Log-confirmed on 2026-04-21: 9 consecutive fallbacks in 8 minutes on a Startup Roundtable episode.
...
- **Fact-checker personas: War / military conflict restraint** (`lib/packs/howard/personas.ts` + `lib/packs/twist/personas.ts`). New **WAR / MILITARY CONFLICT RESTRAINT** section on both The Producer and The Correspondent: they verify casualty numbers, dates, roles, and public statements with **explicit attribution** ("per the IDF…", "per Hamas's health ministry…", "per the UN…"), but never adopt one side's framing as neutral fact, and PASS with `-` when the only available fact-check requires endorsing a combatant's narrative. Scoped to active wars (Gaza, Ukraine, future conflicts); other politics — elections, immigration, climate, culture — remain normal fact-check territory, and The Correspondent's `CLIMATE ANGLE` line is explicitly preserved. Triggered by user feedback on a Gaza fact-check that read as pro-Palestinian advocacy rather than journalism; the narrower war-only scope is Seth's call after the first broader draft overreached.
...
- **Masthead subrail** text: `the morning-radio host | "Four critics. One tab." | Wire` → `Broadsheet | "Four critics. One tab." | Daily`. No pack or person names on the masthead; pack identity still lives in the selector dropdown + feed's persona names.
...
### Release / repo
- **Startup Roundtable Oracle plan privatized.** Reduced `docs/TWIST-ORACLE-PLAN.md` from a 686-line public roadmap plan to a 47-line private exploratory note; removed its references from `docs/INDEX.md` (it was flagged as "v1.7 working spec" based on The Host's off-hand group-chat riff, which wasn't a commitment).
...
- **`Persona.directorHint?` field** on every persona. ~15-token routing hint surfaced in the Smart Director prompt so the LLM routes with intimate knowledge of each voice's specialty (e.g. The Host: *"Confident takes on founder-market fit; cap-table + deal-structure angles. Warm, never mean."*). All 8 shipping personas (the morning-radio host + Startup Roundtable) now carry hints; `directorHint` omission still works — router falls back to `role`. Optional, documented in `docs/SELF-HOST-INSTALL.md` §8 + `docs/BUILD-YOUR-OWN-BACKEND.md` §9.
...
- **`marketing/cws-listing.md` + `SHIP.md`** — Startup Roundtable pack called out in the CWS long description, podcast-first positioning (per `docs/COMPETITIVE-LANDSCAPE-2026-04-18.md`), tightened screenshot captions, five additional search-term slots. Scrubbed the last stale Groq reference.
...
### Changed
- **Troll + Sound FX personas migrated from Groq Llama to xAI Grok 4.1 Fast non-reasoning** (both the morning-radio host and Startup Roundtable packs). Root cause was the Groq free-tier TPD cap on `llama-3.3-70b-versatile` silently returning 429s that read as "persona declined to speak." Anthropic Claude Haiku still powers the Producer + Joker slots (The Producer, The Joke Writer, The Correspondent, The Quant) — those were never affected.
- **The Producer force-react tap now skips pre-stream search entirely.** A tap commits The Producer to speak, and the search round-trip was the single biggest source of latency before any bubble appeared. Search still runs for director-driven cascades where the budget allows it.
...
### Fixed
- **Session-firing deadlock when any upstream hangs.** `/api/transcribe` had a per-session `personasFiring` boolean with no try/finally around the `fireSingle`/`fireAll` await. A stalled Anthropic stream, a stalled xAI stream, or a stuck Brave fetch would leave the flag pinned at `true` forever — killing every subsequent director tick AND every avatar tap on that session until page reload. Root cause behind the v1.4-preview regression where The Producer's avatar animated without responding, then stopped showing a spinner on tap entirely. Fix is layered: (1) Anthropic `messages.stream` now carries `{ signal: AbortSignal.timeout(25_000) }`; (2) `streamXai` accepts an `AbortSignal` and plumbs it into both the initial fetch and its reader loop; (3) both firing branches in the setInterval tick are wrapped in `try { … } finally { personasFiring = false; send("personas_complete") }` so ANY throw or timeout releases the lock.
...
## [1.3.0] — 2026-04-17 — "Startup Roundtable Pack"

The flagship feature: **selectable persona packs.** Keep riding with the morning-radio host crew, or swap in the Startup Roundtable lineup — The Correspondent, The Host, The Reframer, The Quant — with a single dropdown. Pack choice persists across sessions; the Director is pack-agnostic (same archetype slot ids, same behavior) so routing stays identical regardless of who's on the roster. Fully wire-compatible with v1.2.x backends: an unknown pack id gracefully degrades to the morning-radio host server-side via `resolvePack()`, so an older server + newer client still works.

### Added
- **Pack abstraction** (`lib/packs/`). New `Pack` type + `resolvePack(id)` helper that never throws — unknown / missing / malformed ids always resolve to the morning-radio host. Single choke point means the forward-compat matrix (new client + old server, old client + new server, mismatched packs) always degrades cleanly. `PersonaEngine` now takes an optional `pack` in its constructor; legacy call sites that don't pass one continue to get the morning-radio host via a back-compat shim (`lib/personas.ts` re-exports `howardPersonas as personas`).
- **the morning-radio host pack** (`lib/packs/howard/`). Pre-existing The Producer / Troll / The Sound Guy / The Joke Writer personas extracted from `lib/personas.ts` into `lib/packs/howard/personas.ts` with zero content changes. Back-compat shim keeps all v1.2 import sites unchanged.
- **Startup Roundtable pack** (`lib/packs/twist/`). Four new personas, each researched from public Startup Roundtable transcripts and episode clips and mapped to the four archetype slots:
  - **The Correspondent** → `producer` (Fact-Checker). Claude Haiku + Brave. Calm journalistic corrections, "according to" framing, receipts-first.
  - **The Host** → `troll` (Provocateur). Groq Llama 70B. Confident takes, founder-market-fit framing, warm-not-mean, self-aware self-promotion as character fingerprint.
  - **The Reframer** → `soundfx` (The Reframe). Groq Llama 8B. Bracket-delimited sound cues + cultural analogies as primary languages. Reacts to moments, never monologues.
  - **The Quant** → `joker` (Data Comedian). Claude Haiku. Eight joke techniques (NUMBER-AS-PUNCH, COMP-BOMB, BACK-OUT-THE-THING, …) built on data + absurdity.
...
- **Four Startup Roundtable-flavored fixtures** under `scripts/fixtures/director/twist-*.json` exercising each archetype slot with Startup Roundtable vocabulary (cap tables, Series A/B/C, Anthropic, Databricks, a16z / Sequoia name-drops). Same assertions shape as the morning-radio host fixtures — pack-agnostic Director means these validate the same code path with different transcripts.

### Changed
- **`/api/transcribe` + `/api/personas`** now parse an optional `packId` from the request body and pass the resolved pack to `new PersonaEngine({ pack })`. Omitted `packId` → the morning-radio host, so v1.2 clients (and any third-party backend honoring the spec) keep working untouched. The resolved pack id is written into the `session_create` log line for end-to-end correlation.
...
- **`scripts/test-director.ts` gained a `--pack <id>` flag.** Runs the subset of fixtures whose `pack` field matches. Fixtures without a `pack` field default to `"howard"`, so `--pack howard` covers the legacy v1.2 fixtures verbatim. `npm run test:director` with no flag runs all 14 fixtures (10 the morning-radio host + 4 Startup Roundtable) in one pass.
...
### Deferred to a future release
- **Pack-creation installer.** v1.3 ships with the morning-radio host + Startup Roundtable baked in. A proper `pack install` flow (sideload a pack JSON, validate, merge into the dropdown) was scoped out of the flagship and will land in v1.3.1 or v1.4. Adding a pack today still means editing `lib/packs/` and `extension/sidepanel.js::PACKS_CLIENT`.
...
- **v1.3.0 — Startup Roundtable Pack (flagship):** Selectable persona packs. Shipped — see above.
- **v1.4.0 — Smart Director v2:** LLM-assisted routing with rule-based fallback under a 400ms budget.
- **v1.5.0 — Voice + Clip Share:** TTS per persona; highlight/clip export.
- **v2.0.0 — Bobbleheads:** 3D persona avatars with procedural animation.
...
Quality-of-life polish ahead of the Startup Roundtable submission and a security-driven refactor of how the extension handles "demo access."
...
### Changed
- **Persona context rebalanced.** The Director now hands each persona a transcript-heavy context (roughly 90% transcript / 10% cross-persona log) so replies ground in what was actually said, not in what the other personas just said. Fixes The Producer's tendency to echo his own last fact-check.
...
- **Persona character depth.** `lib/personas.ts` received richer character models — specific mannerisms, taboo topics, callback patterns — pulled from the morning-radio host show research.
...
- **The Producer self-repetition** — conversation-context sampler now excludes each persona's own recent messages when building its prompt.
...
First public release. Submitted for the Startup Roundtable $5,000 + guest-spot bounty from The Host and The Reframer.
...
- **Four AI personas**, modeled on the morning show staff and defined in `lib/personas.ts` with deep character research:
  - The Producer (The Producer) — fact-checker. Claude Haiku + Brave Search for live web verification.
  - The Cynical Troll — contrarian commentator. Groq Llama 70B, ~120ms time-to-first-token.
  - The Sound Guy — sound effects / context. Groq Llama 8B.
  - The Joke Writer — comedy writer. Claude Haiku.
...
- **Fact-checking pipeline** for The Producer. Scores sentences for factual claims (numbers, dates, attributions, rankings), runs parallel Brave Search queries, and injects formatted results into the Producer's context.
...
- **Standalone test scripts** — `scripts/test-transcription.ts` and `scripts/test-personas.ts` (with `--fixture` flag for the AstroForge Startup Roundtable transcript).
...
- `personas.ts` is the creative soul of the project. Preserve the morning-radio host character research in any changes.
