...
Tracks in-flight work for the next release after `v1.7.0 "The Fine Print"`. Empty for now — v1.7.0 is the current hard-save state.

## [1.7.0] — 2026-04-21 — "The Fine Print"

**Legal and compliance groundwork for v2.0.** Rewrites the Privacy Policy and Terms of Service end-to-end (lawyer review pending), ships a US-only gate on Peanut Gallery Plus with a three-layer defense (client confirm, server 451, webhook audit), adds a cookie-consent banner on the marketing surfaces, wires Cloudflare Email Routing + Resend for the legal / privacy / security / noreply aliases, and publishes a three-option self-host guide for international users. Also rolls up the Plus subscription scaffold, Broadsheet feed-entry menu, pinned strip, Room Volume dial, quote-card PNG renderer, smart-highlight picker, six-step first-run tutorial, and the two TDZ fixes that shipped to `develop` since v1.6.0.

**Hard save, not released.** Seth elected to cut v1.7.0 as a recoverable checkpoint without rolling to the Chrome Web Store. The release PR is open but not merged. Versioning on manifest + CHANGELOG is bumped so the point-in-time is preserved in Git.

**North Carolina governing law + Macon County forum.** US-only paid tier. International users fork-and-self-host — see [`docs/SELF-HOST-FOR-INTERNATIONAL-USERS.md`](docs/SELF-HOST-FOR-INTERNATIONAL-USERS.md). Tracked-but-open outside the repo: counsel review of the ToS and Privacy drafts ([`docs/legal/LAWYER-REVIEW-BRIEF.md`](docs/legal/LAWYER-REVIEW-BRIEF.md)), completing Cloudflare Email Routing in the dashboard (DNS is in place), and a GitHub issue opened alongside this release inviting contributors to fork the project for non-US deployments.
...
- **Legal docs rewritten end-to-end for lawyer review.** [`docs/legal/TERMS-OF-SERVICE.md`](docs/legal/TERMS-OF-SERVICE.md) (~500 lines) and [`docs/legal/PRIVACY-POLICY.md`](docs/legal/PRIVACY-POLICY.md) (~350 lines) are now counsel-ready drafts. Added: AI-output disclaimer, persona right-of-publicity framing, feedback license, recording-consent warning naming the 10 all-party-consent US states, indemnity, warranty disclaimer + $100 liability floor, auto-renewal disclosure, governing law (North Carolina) + forum (Macon County) + 60-day informal-resolution + jury-trial waiver + small-claims carve-out, survival / severability / force-majeure / assignment / DMCA takedown path. Companion [`docs/legal/LAWYER-REVIEW-BRIEF.md`](docs/legal/LAWYER-REVIEW-BRIEF.md) hands counsel a one-page audit of what's decided vs. what's still for them.
- **Published Terms of Service surface.** New [`app/terms/page.tsx`](app/terms/page.tsx) and `site/terms/index.html` (sibling repo) mirror the privacy-page pattern at `peanutgallery.live/terms` + `www.peanutgallery.live/terms/`. Reader-friendly production versions of the counsel drafts.
- **US-only gate on Peanut Gallery Plus.** Three-layer defense per Terms §1.3: (a) client-side `confirm()` in `extension/sidepanel.js` `openSubscriptionCheckout` that requires affirmative "billing address is in the U.S." before kicking off checkout; (b) backend `app/api/subscription/checkout/route.ts` rejects non-US `country` with HTTP 451 + code `US_ONLY` and redacts email in logs to `se***@hey.com`; (c) `app/api/subscription/webhook/route.ts` reads `customer_details.address.country` from `checkout.session.completed` events and logs non-US completions at warn level as defense-in-depth for Phase 3 Stripe. Drawer markup in `extension/sidepanel.html` now carries a red-stamp-bordered US-only notice above the Subscribe button, pointing international users at the new self-host guide.
- **Cookie-consent banner.** Google Analytics is now gated on both surfaces: [`site/index.html`](../../site/index.html) (vanilla JS + inline styles, broadsheet-aligned) and the Next.js app via [`components/CookieBanner.tsx`](components/CookieBanner.tsx) mounted in [`app/layout.tsx`](app/layout.tsx). First-visit modal with Accept / Decline; choice persisted to `localStorage` under `pg-analytics-consent`; GA script only loads after an affirmative click. `anonymize_ip: true`; no advertising features, no Google Signals.
- **Self-host guide for international users.** [`docs/SELF-HOST-FOR-INTERNATIONAL-USERS.md`](docs/SELF-HOST-FOR-INTERNATIONAL-USERS.md) gives non-US users three options: My-Keys Mode (BYOK, globally available), self-host the Next.js backend in their jurisdiction, or fork + rebrand for a Plus-style product under their own business.
- **Extension drawer legal footer.** `extension/sidepanel.html` now carries Privacy / Terms / Security links under the drawer body, above the Back-to-wire button. Dotted-underline broadsheet styling; opens in new tabs.
...
- **Linear tickets opened + SET-23 / SET-24 completed.** [`SET-23`](https://linear.app/seth-dev/issue/SET-23) (quote-card PNG) + [`SET-24`](https://linear.app/seth-dev/issue/SET-24) (smart-highlight picker) both shipped via daemon-spawned agents and merged in this cycle. [`SET-25`](https://linear.app/seth-dev/issue/SET-25) (persistent subscription identity / SQLite + key generator + admin CLI), [`SET-26`](https://linear.app/seth-dev/issue/SET-26) (Stripe integration), [`SET-27`](https://linear.app/seth-dev/issue/SET-27) (email infrastructure) remain in Backlog pending Seth's Stripe + email provider setup.
